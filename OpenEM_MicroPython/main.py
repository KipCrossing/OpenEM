...
adc2 = pyb.ADC(pyb.Pin.board.Y12)  # create an ADC on pin X2   <<<< CHANGE HERE TO 12
...
while True:
    ...
    if len(rolling_amp) > 4:
        blue_uart.write('%s, %s, %s' % (int(count),int(sum(rolling_amp)/500),round(sum(rolling_sft)/5,2)))
        count += 1
        rolling_amp.pop(0)
        rolling_sft.pop(0)
        rolling_oramp.pop(0)
